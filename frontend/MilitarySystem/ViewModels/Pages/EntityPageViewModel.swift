    ...
    init(service: EntityService<T>, entityFilter: EntityFilter) {
        ...
        $searchText
            ...
            .sink(receiveValue: { searchText in
                self.entityFilter.searchName = searchText
                self.update()
            })
    ...
    func createEntity(_ entity: T) {
        ...
        service.create(entity: entity)
            .sink(receiveCompletion: { completion in
                ...
                switch completion {
                case .finished:
                    NotificationHandler.shared.addNotification(SuccessfulCreateNotificationContent())
                    self.addNewEntity()
                    break
                case .failure(let error):
                    NotificationHandler.shared.addNotification(NetworkRequestErrorNotificationContent(networkRequestError: error))
